...
<html lang="it-IT"> 
    <head>
        <title>GameRig: Case<?php echo $ricerca;?></title>
        <meta charset="utf-8">
    ...
    <body>
        ...
        <main class="case">
            ...
            <div>
                <img src="images/case1.png" alt="Immagine di un Case" id="caseImg">
            </div>
            ...
            <ul>
                ...
                <br>
                Scegli un case compatibile con la tua scheda madre (ATX, micro-ATX, mini-ITX).
                </li>

                <li><span>Flusso d'aria:</span>
                <br>
                Case con mesh frontale e ventole preinstallate garantiscono un raffreddamento ottimale.
                </li>

                <li><span>Gestione dei Cavi:</span>
                <br>
                Verifica la presenza di canaline e spazi dedicati per un'installazione ordinata.
                </li>

                <li><span>Supporto per Dissipatori:</span>
                <br>
                Controlla la compatibilità con dissipatori ad aria o a liquido (AIO).
                </li>

                <li><span>Porte I/O Frontali:</span>
                <br>
                Assicurati che siano presenti le porte necessarie come USB-C e jack audio.
                </li>

                <li><span>Dimensioni GPU e PSU:</span>
                <br>
                Controlla che il case supporti la lunghezza della GPU e il formato dell’alimentatore.
                </li>
            </ul>

            <div id="proposte">
                <div>   
                    <h2>Case più acquistati</h2> 
                    <?php
                        $sql = "SELECT Prodotto.*, COUNT(Acquisto.idProdotto) as acquisti 
                                FROM Prodotto 
                                JOIN Acquisto ON Prodotto.idProdotto = Acquisto.idProdotto 
                                WHERE Prodotto.tipo = 'case' 
                ...
                <div>
                    <h2>Case preferiti dai nostri utenti</h2>
                    <?php
            ...
            <h2>Case in sconto</h2>
            <div id="sconto">
                <?php
                    $sql = "SELECT * FROM Offerta JOIN Prodotto ON Offerta.idProdotto = Prodotto.idProdotto
                            WHERE dataInizio <= CURDATE() AND dataFine >= CURDATE() AND Prodotto.tipo = 'case';";
                    $result = mysqli_query($conn, $sql);
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        echo "<p>Non ci sono Case in sconto</p>";
                    }   
                ?>
            </div>
            <h2>Tutti i nostri Case</h2>
            <div id="case">      
                <?php
                    $sql = "SELECT * FROM Prodotto WHERE tipo = 'case';";
                    $result = mysqli_query($conn, $sql);
                    if (mysqli_num_rows($result) > 0) {
                        while($row = mysqli_fetch_assoc($result)) {
                            echo "<div><a href='prodotto.php?idProdotto=".$row["idProdotto"]."'>";
